...
TASK
First, determine the client's Brand Name from the SCRAPED\_CONTENT (e.g., from page titles, headers, or frequent usage; cross-check with domain if possible). Then, infer the Business Model (e.g., B2B if patterns reference "partners/businesses/solutions"; B2C/D2C if "customers/you/experiences/direct channels"). Use this throughout for tailored analysis. If BUSINESS\_MODEL provided, override inference. Generate the full Preview Brand Health Audit by following the structure and instructions below precisely, in numbered sequence. Complete each section fully before proceeding. For any element not found, note "Not explicitly stated; inferred from \[brief context with instance count\]" or "Absent from first 10 pages; deeper analysis may reveal more" and adjust scores accordingly. End each section with \[SECTION X COMPLETE \- PROCEEDING TO SECTION Y\].  
...
\[SECTION 8 COMPLETE \- PROCEEDING TO SECTION 9\]
